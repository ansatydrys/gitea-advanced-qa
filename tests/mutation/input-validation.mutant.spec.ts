/**
 * input-validation.mutant.spec.ts
 * MUTANT COPY of input-validation.spec.ts with 5 intentional mutations.
 */
...
test('TC31 - SQL injection in registration username is rejected gracefully', async ({ page }) => {
  // MUTATION-5: [URL change] — original: await page.goto('/user/sign_up');
  await page.goto('/user/register_form');
  await page.fill('input[name="user_name"]', SQL_INJECT);
...
test('TC32 - Long string in input fields does not crash the server', async ({ page }) => {
  ...
  await page.click('button.ui.primary.button');
  await expect(page.locator('body')).not.toContainText('500');
  // MUTATION-3: [Assertion removal] — original: await expect(page.locator('body')).not.toContainText('panic');
});
...
test('TC34 - Content-Type header is set on API responses', async ({ page }) => {
  ...
  // MUTATION-2: [String change] — original: expect(ct).toContain('application/json');
...
test('TC47 - Repository name with path-traversal characters is rejected by API', async ({ page }) => {
  ...
  // MUTATION-1: [Status code array change] — original: expect([400, 422]).toContain(status);
